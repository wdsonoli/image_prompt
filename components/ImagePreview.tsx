...
export const ImagePreview: React.FC<ImagePreviewProps> = ({ 
    ...
    return (
        <div 
            ref={containerRef}
            className="relative group rounded-xl overflow-hidden border border-slate-700 bg-slate-900 shadow-2xl w-full h-[340px] sm:h-[450px] md:h-[500px] flex items-center justify-center bg-slate-950/50 select-none touch-none"
            onMouseLeave={onEnd}
            onMouseUp={onEnd}
            onTouchEnd={onEnd}
        ...
        >
            ...
            <div className="absolute top-3 left-3 flex flex-col gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity z-40">
                ...
                <button 
                    ...
                    className={`p-2 sm:p-2.5 rounded-xl flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-semibold backdrop-blur-md border transition-all ${isRemovingBackground ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-900/90 border-slate-700 text-slate-300 hover:bg-slate-800'}`}
                >
                    <Scissors size={15} />
                ...
                <button 
                    ...
                    className={`p-2 sm:p-2.5 rounded-xl flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-semibold backdrop-blur-md border transition-all ${isCropping ? 'bg-orange-600 border-orange-500 text-white shadow-lg' : 'bg-slate-900/90 border-slate-700 text-slate-300 hover:bg-slate-800'}`}
                >
                    <CropIcon size={15} />
            ...
            {isCropping && (
                <div className="absolute bottom-4 inset-x-2 sm:inset-x-4 flex flex-col gap-2 z-50">
                    <div className="flex justify-center gap-2">
                         <button 
                            ...
                            className={`p-2.5 sm:p-3 rounded-full border transition-all ${cropShape === 'rect' ? 'bg-blue-500 border-blue-400 text-white' : 'bg-slate-900/90 border-slate-700 text-slate-400'}`}
                        >
                            <Square size={18} />
                        </button>
                        <button 
                            ...
                            className={`p-2.5 sm:p-3 rounded-full border transition-all ${cropShape === 'circle' ? 'bg-blue-500 border-blue-400 text-white' : 'bg-slate-900/90 border-slate-700 text-slate-400'}`}
                        >
                            <Circle size={18} />
                        </button>
                    </div>
                    
                    <div className="flex items-center gap-2 sm:gap-3 bg-slate-900/95 backdrop-blur-xl px-3 sm:px-4 py-2 sm:py-3 rounded-2xl border border-slate-700 shadow-2xl">
                        <button 
                            onClick={applyCrop}
                            className="flex-1 flex items-center justify-center gap-2 text-xs sm:text-sm font-black text-white bg-blue-600 hover:bg-blue-500 py-2.5 sm:py-3 rounded-xl transition-all uppercase tracking-widest active:scale-95"
                        >
                            <Check size={16} /> Confirmar
                        </button>
                        <button 
                            onClick={() => setIsCropping(false)}
                            className="p-2.5 sm:p-3 bg-slate-800 text-slate-300 rounded-xl hover:text-white transition-all active:scale-95"
                        >
                            <X size={18} />
                        </button>
                    </div>
            ...
            {/* Footer Navigation - Always accessible or mobile optimized */}
            {!isCropping && (
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 sm:gap-3 bg-slate-900/90 backdrop-blur-md px-3 sm:px-5 py-1.5 sm:py-2.5 rounded-full border border-slate-700 shadow-xl opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-300 translate-y-0 sm:translate-y-2 sm:group-hover:translate-y-0 z-10">
                    <button onClick={handleZoomOut} disabled={scale <= 1} className="p-1.5 sm:p-2 hover:bg-slate-700 rounded-full text-slate-300 disabled:opacity-50 transition-colors">
                        <ZoomOut size={16} />
                    </button>
                    <span className="text-[11px] sm:text-xs font-mono text-slate-400 w-10 sm:w-12 text-center font-bold">{Math.round(scale * 100)}%</span>
                    <button onClick={handleZoomIn} disabled={scale >= 4} className="p-1.5 sm:p-2 hover:bg-slate-700 rounded-full text-slate-300 disabled:opacity-50 transition-colors">
                        <ZoomIn size={16} />
                    </button>
                    <div className="w-px h-4 sm:h-5 bg-slate-700 mx-0.5 sm:mx-1"></div>
                    <button onClick={handleReset} className="p-1.5 sm:p-2 hover:bg-slate-700 rounded-full text-slate-300 transition-colors">
                        <RotateCcw size={16} />
                    </button>
                </div>
            )}
            ...
            {!isCropping && (
                <button 
                    onClick={onRemove}
                    className="absolute top-3 right-3 z-20 p-2 sm:p-2.5 bg-red-500/90 text-white rounded-full opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity hover:bg-red-600 shadow-lg active:scale-90"
